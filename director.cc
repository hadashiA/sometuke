...
void Director::Sample() {
    GLProgram p;
    if (p.LoadShaderFiles("position_ucolor.vert", "position_ucolor.frag")) {
        IIINFO("ok");
    }
    int color_location     = glGetUniformLocation(p.handle(), "u_Color");
    int pointsize_location = glGetUniformLocation(p.handle(), "u_PointSize");
    
    p.Use();
    // p.SetUniformForModelViewProjectionMatrix();
